...
class Tracking:
    ...
    def __init__(self, project_name, experiment_name, default_backend: str | list[str] = "console", config=None):
        ...
        if "tracking" in default_backend or "wandb" in default_backend:
            ...
            print("[W&B DEBUG] pid =", os.getpid(), flush=True)
            print("[W&B DEBUG] WANDB_ENTITY env =", os.environ.get("WANDB_ENTITY"), flush=True)
            print("[W&B DEBUG] WANDB_PROJECT env =", os.environ.get("WANDB_PROJECT"), flush=True)
            print("[W&B DEBUG] project_name arg =", project_name, flush=True)
            print("[W&B DEBUG] experiment_name arg =", experiment_name, flush=True)
            print("[W&B DEBUG] entity used =", entity, flush=True)

            entity = os.environ.get("WANDB_ENTITY") or "echo2"
            project = os.environ.get("WANDB_PROJECT") or project_name
            ...
            print("[W&B DEBUG] run.entity =", getattr(run, "entity", None), flush=True)
            print("[W&B DEBUG] run.project =", getattr(run, "project", None), flush=True)
            print("[W&B DEBUG] run.name =", getattr(run, "name", None), flush=True)
            print("[W&B DEBUG] run.url =", getattr(run, "url", None), flush=True)
