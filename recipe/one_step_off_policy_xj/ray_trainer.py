...
class OneStepOffRayTrainer(RayPPOTrainer):
    ...
    def __init__(
        ...
        if self.enable_adaptive_sync:
            ...
            print(f"[Multi-Step Off-Policy] Adaptive mode enabled, init_sync_frequency={self.sync_frequency}")
        else:
            # 固定模式（方案2：降低同步频率）
            self.adaptive_sync = None
            self.sync_frequency = config.actor_rollout_ref.rollout.get("sync_frequency", 1)
            print(f"[Multi-Step Off-Policy] Fixed mode, sync_frequency={self.sync_frequency}")
