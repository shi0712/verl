...
class OneStepOffRayTrainer(RayPPOTrainer):
    ...
    def __init__(
        ...
        self.sync_frequency = int(config.actor_rollout_ref.rollout.get("sync_frequency", 1))
        if self.sync_frequency < 1:
            raise ValueError(
                "actor_rollout_ref.rollout.sync_frequency must be >= 1, "
                f"got {self.sync_frequency}"
            )
    ...
    async def _sync_rollout_weights_if_needed(self, force: bool = False) -> bool:
        should_sync = force or (
            self.actor_update_steps > 0 and self.actor_update_steps % self.sync_frequency == 0
        )
        if not should_sync:
            return False
    ...
    async def _async_gen_next_batch(self, continuous_iterator):
        ...
        _gen_msg = f"\n[GEN] step={self.global_steps} | prompts={num_prompts} | n={n_rollouts} | total_seqs={num_prompts * n_rollouts}"
        print(_gen_msg, flush=True)
        if hasattr(self, '_perf_log'):
            self._perf_log.write(_gen_msg + "\n")
        ...
        resp_tokens = int(resp_mask.sum())
        seq_lens = attn.sum(dim=-1).float()
        resp_lens = attn[:, prompt_len:].sum(dim=-1).float()
        gen_time = timing_raw.get("generate_async", 0)
        tok_per_sec = total_tokens / gen_time if gen_time > 0 else 0
        _gen_done_msg = (
            f"[GEN] done | generate_async={gen_time:.1f}s | "
            f"total_tokens={total_tokens:,} | resp_tokens={resp_tokens:,} | "
            f"seq_len avg={seq_lens.mean():.0f} min={seq_lens.min():.0f} max={seq_lens.max():.0f} | "
            f"resp_len avg={resp_lens.mean():.0f} min={resp_lens.min():.0f} max={resp_lens.max():.0f} | "
            f"throughput={tok_per_sec:,.0f} tok/s"
        )
        print(_gen_done_msg, flush=True)
        if hasattr(self, '_perf_log'):
            self._perf_log.write(_gen_done_msg + "\n")
            self._perf_log.flush()
    ...
    async def fit(self):
        ...
        _log_dir = os.path.join(
            os.environ.get("RAY_DATA_HOME", os.path.expanduser("~/verl")),
            "perf_logs",
        )
        os.makedirs(_log_dir, exist_ok=True)
        _log_path = os.path.join(
            _log_dir,
            f"{self.config.trainer.experiment_name}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.log",
        )
        self._perf_log = open(_log_path, "w")
        self._perf_log.write(f"Perf log started at {datetime.datetime.now()}\n")
        self._perf_log.write(f"Log file: {_log_path}\n\n")
        self._perf_log.flush()
        print(f"[PERF] Writing detailed perf log to: {_log_path}", flush=True)
        ...
        await self._sync_rollout_weights_if_needed(force=True)
        ...
        while batch_data_future is not None:
            ...
            with marked_timer("step", timing_raw):
                ...
                with marked_timer("sync_rollout_weights", timing_raw, color="purple"):
                    synced_rollout_weights = await self._sync_rollout_weights_if_needed()
                metrics["rollout/synced"] = float(synced_rollout_weights)
            ...
            import resource
            _t = {k: v for k, v in timing_raw.items()}
            _bsz = len(batch.batch) if hasattr(batch, 'batch') else 0
            _total_tok = int(batch.batch["attention_mask"].sum()) if hasattr(batch, 'batch') else 0
            _resp_tok = int(batch.batch["response_mask"].sum()) if hasattr(batch, 'batch') else 0
            try:
                _cpu_mem = resource.getrusage(resource.RUSAGE_SELF).ru_maxrss / 1024 / 1024  # GB
            except Exception:
                _cpu_mem = metrics.get("perf/cpu_memory_used_gb", -1)

            # ── config vs actual sampling parameters ──
            _cfg = self.config
            _cfg_train_bs = _cfg.data.train_batch_size
            _cfg_n = _cfg.actor_rollout_ref.rollout.n
            _cfg_mini_bs = _cfg.actor_rollout_ref.actor.ppo_mini_batch_size
            _cfg_micro_bs = _cfg.actor_rollout_ref.actor.ppo_micro_batch_size_per_gpu
            _cfg_rollout_micro = _cfg.actor_rollout_ref.rollout.get("log_prob_micro_batch_size_per_gpu", "N/A")
            _cfg_ref_micro = _cfg.actor_rollout_ref.ref.get("log_prob_micro_batch_size_per_gpu", "N/A")
            _n_train_gpus = _cfg.trainer.n_gpus_per_node * _cfg.trainer.nnodes
            _n_rollout_gpus = _cfg.rollout.n_gpus_per_node * _cfg.rollout.nnodes
            _actual_prompts = _bsz // _cfg_n if _cfg_n > 0 else _bsz
            _actual_total_seqs = _bsz
            _num_mini_batches = _actual_total_seqs // _cfg_mini_bs if _cfg_mini_bs > 0 else 1
            _num_micro_per_mini = _cfg_mini_bs // (_cfg_micro_bs * _n_train_gpus) if (_cfg_micro_bs * _n_train_gpus) > 0 else 1

            _step_time = _t.get("step", 1)
            _phases = [
                "generate_async", "gen", "sync_rollout_weights",
                "reward", "old_log_prob", "ref", "values",
                "adv", "update_critic", "update_actor", "testing",
            ]
            _lines = []
            _lines.append(f"{'=' * 80}")
            _lines.append(f"  STEP {self.global_steps} SUMMARY")
            _lines.append(f"{'=' * 80}")
            _lines.append(f"  SAMPLING PARAMETERS (config -> actual):")
            _lines.append(f"  {'─' * 52}")
            _lines.append(f"  {'train_batch_size':<35s} cfg={_cfg_train_bs:<6}  actual_prompts={_actual_prompts}")
            _lines.append(f"  {'rollout.n':<35s} cfg={_cfg_n:<6}  actual_total_seqs={_actual_total_seqs}")
            _lines.append(f"  {'ppo_mini_batch_size':<35s} cfg={_cfg_mini_bs:<6}  num_mini_batches={_num_mini_batches}")
            _lines.append(f"  {'ppo_micro_batch_size_per_gpu':<35s} cfg={_cfg_micro_bs:<6}  micro_steps/mini={_num_micro_per_mini}")
            _lines.append(f"  {'rollout.log_prob_micro_bs/gpu':<35s} cfg={_cfg_rollout_micro}")
            _lines.append(f"  {'ref.log_prob_micro_bs/gpu':<35s} cfg={_cfg_ref_micro}")
            _lines.append(f"  {'trainer GPUs':<35s} {_n_train_gpus}")
            _lines.append(f"  {'rollout GPUs':<35s} {_n_rollout_gpus}")
            _lines.append(f"  {'─' * 52}")
            _lines.append(f"  Batch size:       {_bsz} seqs")
            _lines.append(f"  Total tokens:     {_total_tok:,}")
            _lines.append(f"  Response tokens:  {_resp_tok:,}")
            _lines.append(f"  GPU mem alloc:    {metrics.get('perf/max_memory_allocated_gb', -1):.2f} GB")
            ...
            for _p in _phases:
                if _p in _t:
                    _pct = _t[_p] / _step_time * 100 if _step_time > 0 else 0
                    _lines.append(f"  {_p:<30s} {_t[_p]:>10.1f} {_pct:>9.1f}%")
            _lines.append(f"  {'─' * 52}")
            _lines.append(f"  {'TOTAL (step)':<30s} {_step_time:>10.1f}")
            _lines.append(f"  Throughput:       {metrics.get('perf/throughput', 0):,.0f} tok/s")
            _lines.append(f"{'=' * 80}")

            _summary_text = "\n".join(_lines)
            print(f"\n{_summary_text}\n", flush=True)
            self._perf_log.write(f"\n{_summary_text}\n\n")
            self._perf_log.flush()
